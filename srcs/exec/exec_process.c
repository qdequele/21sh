...
static void	assert_file(int i, char *target, int mode)
{
	...
	{
		print_err("21sh: Is a directory: ", target);
		close(i);
	}
	flag = (mode == O_RDONLY) ? R_OK : W_OK;
	if (stat(target, &sb) == 0 && access(target, flag) != 0)
	{
		print_err("21sh: Permission denied: ", target);
		close(i);
	}
}
...
static void	get_new_stdio(t_process *p, t_io_channel *s)
{
	...
	while (++i < 3)
	{
		...
		{
			print_err("21sh: No such file or directory", "");
			exit(1);
		}
...
void		launch_process(t_process *p, pid_t pgid, int foreground)
{
	...
	if (stat(p->argv[0], &sb) == 0 && access(p->argv[0], X_OK) != 0)
		print_err("21sh: Permission denied: ", p->argv[0]);
	else
		print_err("21sh: command not found: ", p->argv[0]);
	exit(127);
}
